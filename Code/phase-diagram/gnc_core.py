...
def apply_prestige_bias(tau, delta, src, indptr, deg, ps, eta, tiebreak,
    ...
    """
    tau_out = np.empty_like(tau)
    delta_out = np.empty_like(delta)
    ...
    for i in range(indptr.size - 1):
        ...
        s = ps * ktil + (1.0 - ps) * eta[a:b]
        src_order = np.lexsort((tiebreak[a:b], -s))     # most prestigious first
        pair_order = np.argsort(-t, kind="stable")      # highest trust first
        tau_out[a + src_order] = t[pair_order]
        delta_out[a + src_order] = d[pair_order]
...
# ---------------------------------------------------------------------------
def degree_biased_seeding(b_init, deg, x):
    ...
    H = np.argsort(-deg, kind="stable")[:h]      # degree ties -> ascending id
    P = np.argsort(-b_init, kind="stable")[:h]   # the h most positive beliefs
